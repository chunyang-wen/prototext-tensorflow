...
class Generator {
 public:
  Generator(const std::string& tf_header_prefix)
 ...
 private:
  ...
  void AddNamespaceToCurrentSection(const std::string& package, bool open);
...
string GetPackageReferencePrefix(const FileDescriptor* fd) {
  std::string result = "::";
  const std::string& package = fd->package();
...
// Returns the std::string that can be used for a header guard for the generated
...
void Generator::AppendEnumFunctions(const EnumDescriptor& enum_d) {
  const std::string sig = StrCat("const char* ", GetEnumNameFn(enum_d), "(\n    ",
...
void Generator::AppendParseMessageFunction(const Descriptor& md) {
  ...
  for (int i = 0; i < md.field_count(); ++i) {
    ...
    if (field->is_repeated()) {
      ...
      const std::string oneof_name = field->containing_oneof()->name();
    ...
    if (field->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE) {
      ...
      for (int enum_i = 0; enum_i < enum_d->value_count(); ++enum_i) {
        ...
        const std::string& value_name = value_d->name();
...
void Generator::AppendDebugStringFunctions(const Descriptor& md) {
  ...
  // Append the two debug std::string functions for <md>.
...
void Generator::AddNamespaceToCurrentSection(const std::string& package, bool open) {
  Print();
